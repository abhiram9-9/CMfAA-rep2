...
1004 format(a25,1x,1pe12.5)
 ...
 subroutine special_source(qdt,ixI^L,ixO^L,iw^LIM,qtC,wCT,qt,w,x)
   ...
   ! write(*,*) "Temp @ 5,5 = ",TField(5,5)
   ! tmp(5,1:6)= Lambda([1d3,1d4,1d5,1d6,1d7,1d8])
   ! write(*,*) "Lambda @ 5,5= ", tmp(5,1:6)
   ! tmp(5,6) =  Lambda(TField(5,6)) *(w(5,6,rho_)*lconv)**2*qdt*unit_time/unit_pressure 


   ! write(*,*) "energy radiated : " , tmp(5,6)
   ! write(*,*) "energy added:" , qdt*Edot*eflow1(qt)
   ! write (*,*) "UNIT_DENS*UNIT_TIME: " ,unit_density*unit_time
   ! write(*,*) "UNIT_DENSITY/UNIT_PRESSURE", unit_density/unit_pressure, w(5,5,rho_)
